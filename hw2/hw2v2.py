...
class MultiLayerPercepModel:
	...
	def buildModel(self):
		...
		for ii in range(1, len(self.layers)-1):
			self.yhat = tf.nn.relu(tf.add(tf.matmul(self.yhat, weights['h%d' % ii]), biases['b%d'%ii]));

		self.yhat = tf.sigmoid(tf.add(tf.matmul(self.yhat, weights['h%d'% (len(self.layers)-1)]), biases['b%d' % (len(self.layers)-1)]));
